...
import { getMonitorEvaluationDetail, submitMonitorEvaluationReview } from '../../../api/evaluationApi';
import { getAllCriteria } from '../../../api/evaluationApi';
import '../../../styles/MonitorClass.css';

export default function MonitorReviewModal({ evaluationId, studentName, studentCode, isReadOnly, onClose, onSuccess }) {
  ...
  const handleScoreChange = (code, value, maxScore) => {
    ...
    if (maxScore !== undefined && parsedValue > maxScore) parsedValue = maxScore;
    setAdjustedScores(prev => ({ ...prev, [code]: parsedValue }));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await submitMonitorEvaluationReview({
        evaluationId,
        adjustedDetails: adjustedScores
      });
      onSuccess();
    } catch (err) {
  ...
  const totalGroupMax = useMemo(() => {
    return criteriaList.reduce((acc, c) => acc + (c.maxPoint || 0), 0);
  }, [criteriaList]);
      ...
      <div className="mrm-container" onClick={e => e.stopPropagation()}>
        ...
        <div className="mrm-header">
          <div>
            <h2 className="mrm-title">
              {isReadOnly ? 'Xem phiếu: ' : 'Duyệt phiếu: '}
              {studentName}
            </h2>
            <p className="mrm-subtitle">MSSV: {studentCode}{isReadOnly ? ' — Chỉ xem' : ' — Có thể điều chỉnh điểm sinh viên'}</p>
          </div>
          ...
          {loading ? (
            ...
            <>
              <div className="mrm-group-header">
                <span className="material-symbols-outlined" style={{ color: '#ef4444' }}>assignment</span>
        ...
        <div className="mrm-footer">
          <div className="mrm-footer-info">
            <span className="material-symbols-outlined" style={{ fontSize: '18px', color: isReadOnly ? '#64748b' : '#ef4444' }}>
              {isReadOnly ? 'visibility' : 'info'}
            </span>
            <span>{isReadOnly ? 'Phiếu này chỉ được xem, không thể chỉnh sửa.' : 'Kiểm tra kỹ trước khi phê duyệt.'}</span>
          </div>
